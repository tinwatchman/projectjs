...
var ProjectJsCompiler = function() {
    ...
    this.buildProject = function(project, registry) {
        ...
        if (project.hasBuildDir()) {
            isFolderBuild = true;
            buildFolder = project.getBuildDir();
            if (!path.isAbsolute(buildFolder)) {
                buildFolder = path.resolve(project.getRootDir(), project.getBuildDir());
            }
        }
        if (project.hasSrcDir()) {
            isSrcFolder = true;
            srcFolder = project.getSrcDir();
            registry.isAddingSrcDir(false);
            if (!path.isAbsolute(srcFolder)) {
                srcFolder = path.resolve(project.getRootDir(), project.getSrcDir());
            }
        }
