...
var ProjectJsCompiler = function() {
    ...
    this.buildProject = function(registry, projectRoot, buildFolder, startInfo) {
        ...
        for (var fullName in classMap) {
            ...
            this.compileClassFile(compileArgs);
        }
        ...
        if (_.isString(startInfo) && isFolderBuild) {
            ...
            // that's required is to copy the start script to the build 
            // folder.
            var startPath = path.join(projectRoot, startInfo),
                copyStartPath = path.join(buildFolder, startInfo);
            fs.ensureDirSync(path.dirname(copyStartPath));
            fs.copySync(startParth, copyStartPath);
