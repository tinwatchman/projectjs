...
var ProjectJsParser = function() {
     ...
     */
    this.createRegistry = function(project, options) {
        var args;
        if (project instanceof ProjectJsFile) {
            ...
            if (project.hasSrcDir()) {
                ...
                args["addSrcDir"] = _.has(options, "addSrcDir") ? options.addSrcDir : true;
            }
        } else {
        ...
        // set options
        if (!_.has(args, "addSrcDir") && _.has(options, "addSrcDir")) {
            args["addSrcDir"] = options["addSrcDir"];
        }
        args["compileSuffix"] = _.has(options, "compileSuffix") ? options["compileSuffix"] : ".tmp";
        args["addCompileSuffix"] = _.has(options, "addCompileSuffix") ? options["addCompileSuffix"] : false;
        // create registry
        var registry = new ProjectJsRegistry(args);
